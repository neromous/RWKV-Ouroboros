...
from custom_optimizers.sgd import SGD
import copy
...
class BlockStateList:
    def __init__(self, shift_states, wkv_states):
        self.wkv_states = nn.Parameter(wkv_states)
        self.shift_states = nn.Parameter(shift_states)

    @staticmethod
    def create(N, B, C, n_head, head_size, device, dtype):
        result = BlockStateList.empty(N, B, C, n_head, head_size, device, dtype)
        result.wkv_states.data[:] = 0
        result.shift_states.data[:] = 0
        return result

    @staticmethod
    def empty(N, B, C, n_head, head_size, device, dtype):
        wkv_states = nn.Parameter(torch.empty(
            (N, B, n_head, head_size, head_size),
            device=device,
            dtype=torch.float,
        ))
        shift_states = nn.Parameter(torch.empty((N, 2, B, C), device=device, dtype=dtype))
        return BlockStateList(shift_states, wkv_states)

    def __getitem__(self, layer: int):
        return BlockState(
            (self.shift_states.data[layer, 0], self.wkv_states.data[layer]),
            (self.shift_states.data[layer, 1]),
        )

    def __setitem__(self, layer: int, state: BlockState):
        self.shift_states.data[layer, 0] = state.time_mix_state[0]
        self.wkv_states.data[layer] = state.time_mix_state[1]
        self.shift_states.data[layer, 1] = state.channel_mix_state
