...
import numpy as np
from .model import RWKV, BlockStateList, deepspeed_checkpoint, L2Wrap
import copy
import sys


def ppl(token, logits):
    with torch.no_grad():
        cel = F.cross_entropy(
            logits.view(-1, logits.size(-1)),
            torch.tensor([token], device=logits.device),
            reduction="none",
        )
...
def speak_next_token(
    ...
    if use_pos_cfg:
        if pos_cfg_state is None:
            new_states_pos = BlockStateList.create(
                rwkv.args.n_layer,
                1,
                rwkv.args.n_embd,
                rwkv.args.n_head,
    ...
    if use_neg_cfg:
        if neg_cfg_state is None:
            new_states_neg = BlockStateList.create(
                rwkv.args.n_layer,
                1,
                rwkv.args.n_embd,
                rwkv.args.n_head,
...
def train_forward(rwkv, batch: dict, states: BlockStateList = None):
    # pre calc
    seq = batch["input_ids"]
