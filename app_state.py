...
config = load_config('./resources/config/state_tuning.json')
...
if config.trainer.optimzer_style in ["adam","adamw" ]:
    from deepspeed.ops.adam import DeepSpeedCPUAdam, FusedAdam

from model.v6.rwkv_state_tuing.model import RWKV
from model.v6.rwkv_state_tuing.block import BlockStateList
from model.v6.rwkv_state_tuing.functions import train_forward, speak, ppl
...
def build_dynic_len_seq(seq):
    res = []
    while len(seq) > 0:
        step = random.randrange(512, 2048)
        res.append(seq[:step])
        seq = seq[step:]
    return res
...
class Robot:
    def __init__(self):
        "docstring"
        self.config = config
        self.train_tokenizer = prefix_tokenizer(file_name=self.config.inference.tokenizer)
        self.infer_tokenizer = prefix_tokenizer(file_name=self.config.inference.tokenizer)
        self.ds_config = {
        ...
        self.top_p = 0.2
        self.alpha_frequency = 0.35
        self.alpha_presence = 0.35
    ...
    def save_state_to_disk(self, n="default"):
        fpath = f"{self.config.proj_dir}/states/{n}.pth"
        ego_path = f"{self.config.proj_dir}/states/ego-{n}.pth"
    ...
    def load_state_from_disk(self, n="default"):
        try:
            ...
            self.current_states = BlockStateList(data.shift_states, data.wkv_states)
        except:
            print("未能成功初始化state")

        try:
            fpath = f"{self.config.proj_dir}/states/ego-{n}.pth" 
            data = torch.load(fpath)
            self.ego = BlockStateList(data.shift_states, data.wkv_states)
        except:
    ...
    def save_weight_to_disk(self, n="default"):
        fpath = f"{self.config.proj_dir}/{n}.pth"
    ...
    def model_online(self):
        self.optimizer =  DeepSpeedCPUAdam(
            ...
            lr=self.config.trainer.lr_init,
            betas=(self.config.trainer.beta1, self.config.trainer.beta2),
            eps=self.config.trainer.adam_eps,
            adamw_mode=self.config.trainer.adamw_mode,
        ...
        self.lr_scheduler = deepspeed.runtime.lr_schedules.WarmupLR(
            self.optimizer,
            warmup_min_lr=0.2 * self.config.trainer.lr_init,
            warmup_max_lr=self.config.trainer.lr_init,
            warmup_num_steps=self.config.trainer.warmup_steps,
            warmup_type="linear",
        )
    ...
    def speak(self,
        ...
        with torch.no_grad():
            ...
            return speak_tokens, speak_str, out_state
    ...
    def reflex(self, inputs, state, reset_by_step=True):
        ...
        with torch.no_grad():
            ...
            for tokens in all_tokens:
                ...
                while len(tokens) > 0:
                    i += 1
                    output = tokens[:self.config.trainer.ctx_len]
                    tokens = tokens[self.config.trainer.ctx_len - self.config.trainer.window:]
    ...
    def learn(self,
        ...
        ctx_len = self.config.trainer.ctx_len
        ...
        for tokens in all_tokens:
            ...
            while len(tokens) > 0:
                i += 1
                ctx_len = random.randrange(int(self.config.trainer.ctx_len / 8)  , self.config.trainer.ctx_len)
                output = tokens[:ctx_len]
                tokens = tokens[ctx_len - window:]
                if len(output) < 16:
                    continue
                batch = {"input_ids": output, "masks": None}
    ...
    def db_transact(self, domain, title):
        query = {}
        url = self.config.db.host + self.config.db.api_for_transact
        resp = requests.post(url, json=query)
        text = resp.json()['content']
        return text

    def db_query(self, domain, title, content):
        query = {}
        url = self.config.db.host + self.config.db.api_for_query
        resp = requests.post(url, json=query)
        text = resp.json()['content']
        return text
