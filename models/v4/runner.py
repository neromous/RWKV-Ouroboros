...
class RWKV_RNN(nn.Module):
    def __init__(self,weight,dtype="bf16"):
        super().__init__()
        if dtype == "fp32":
            self.my_datatype= torch.float32
        elif dtype == "fp16":
            self.my_datatype= torch.float16
        elif dtype == "bf16":
    ...
    def forward(self, token, state):
        with torch.no_grad():
            if state == None:
                state = torch.zeros(self.args.n_layer * 5,
                                    self.args.n_embd).to('cuda',dtype=self.my_datatype)
