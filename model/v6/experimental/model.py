...
class TimeMix(nn.Module):
    ...
    def __init__(self, layer_id, n_layer, n_embd, n_head, head_size, dim_att, chunk_len:int = 24, precision:int = 64, max_ctx_len:int = 4096):
        ...
        self.n_kv_head_ratio = 1
        self.n_kv_head = self.n_kv_head_ratio * self.n_head
        self.dim_rk = int(1 * self.n_embd)
        self.dim_v = int(1 * self.n_embd)
        self.r_head_size = self.dim_rk // self.n_head
        self.k_head_size = self.dim_rk // self.n_head
        self.v_head_size = self.dim_v // self.n_head

        assert self.dim_rk % self.n_head == 0
        assert self.dim_rk % self.n_kv_head == 0
        assert self.dim_v % self.n_kv_head == 0
    ...
    def forward(self, x, kv_state=None):
        ...
        # padding to support fast path for no-exact chunk size multiple sequeence
        n_padding = (self.chunk_len - x.size(-2) % self.chunk_len) % self.chunk_len
        if n_padding != 0:
        ...
        x_logits, wkv_state = rwkv_inner(r, k, v, w, u, kv_state, self.chunk_len)
        x_logits = x_logits.to(dtype=torch.bfloat16)
        # x_logits = x_logits.transpose(1,2).reshape(B,T,C)
        x_logits = x_logits.transpose(1,2).reshape(B*T, H*V)

        # Reshape and normalize the logits
        # x_logits = x_logits.view(-1, C)
        x_logits = self.ln_x(x_logits / self.head_size_divisor).view(B, T, H*V)
        x_logits = self.output(x_logits * g)
        if n_padding != 0:
            x_logits = x_logits[..., :-n_padding, :]
...
class Block(nn.Module):
    ...
    def __init__(self, layer_id, n_layer, n_embd, n_head, head_size, dropout, dim_att, dim_ffn, chunk_len=24):
        ...
        self.att = TimeMix(layer_id, n_layer, n_embd, n_head,
                           head_size, dim_att, chunk_len=chunk_len)
    ...
    def forward(self, x, state=None):
        ...
        att_out, att_state = self.att(self.ln1(x), state)
...
class RWKV(nn.Module):
    ...
    def forward(self, idx, states=None):
        ...
        new_states = [None for x in range(len(self.blocks))]

        if states is None:
            states = [None for x in range(len(self.blocks))]
        else:
            states = [state.to("cuda") for state in states]
        ...
        for i in range(len(self.blocks)):
            ...
            new_states[i] = state.detach().to('cpu')
        x = self.ln_out(x)
        logits = self.head(x)
