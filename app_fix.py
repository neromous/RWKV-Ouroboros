...
class RwkvConfig:
    ...
    def new(cls,args:dict, **kwargs):
        m = cls()
        for p, f, dv in cls.valid_props():
            setattr(m, p, f(args.get(p,dv)))

        for p, f, dv in cls.valid_props():
            setattr(m, p, f(kwargs.get(p,dv)))
        return m
...
args = RwkvConfig.new(config, **config['model'])
...
def infer_model_load():
    global rnn_model, infer_state, model_engine
    req = request.json
    req = dict(request.json)
    print("=========")
    rnn_model = RWKV_RNN(model_engine.module.state_dict(),args)
    #rnn_model = model_engine
    infer_state = None
    return {"message": "success"}
