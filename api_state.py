from app_lora import Robot, config
...
def speak():
    ...
    token_stop = req.get('token_stop', [22,23,65530,65535])
    void_token = req.get('void_token', 11)
    temperature = req.get('temperature', 0.2)
    top_p =  req.get('top_p', 0.2)
    decay =  req.get("decay", 0.95)
    ctx_len = req.get('ctx_len', 512)
    app.temperature = temperature
    app.top_p = top_p
    print("------------------------------------------------------")
    print(f"\n\n温度:{temperature}  top_p:{top_p} ctx_len:{ctx_len}")
    ...
    if app.current_states is not None:
        state = app.current_states.decay()
    else:
        state = None
...
def learn_by_text():
    ...
    token = app.infer_tokenizer.encode(text)
    tokens = [token]
    if debug:
        print("--------------------------debug----------------------------") 
        print(tokens)
        ...
        print("---------------------------debug---------------------------")
    losses = app.learn(tokens, app.current_states)
